...
\section{O Teorema Fundamental para Integral de Linha \index{teorema fundamental para integral de linha}}
\begin{teo}Seja $\vec{F}$ um campo vetorial definido numa região $R$ do espaço e $P_0(x_0,y_0,z_0)$ e $P_1(x,y,z)$ dois pontos em $R$. Então $\vec{F}$ é conservativo \index{campo!conservativo}, isto é, $\vec{F}=\vec{\nabla}\varphi$ para algum potencial $\varphi$, se e somente se
...
\end{teo}
\begin{proof}
Sem perda de generalidade, suponha $C$ uma curva suave parametrizada por $\vec{r}(t)=x(t)\vec{i}+y(t)\vec{j}+z(t)\vec{k}$, $t_0\leq t\leq t_1$, com início em $\vec{r}(t_0)=P_0$ e fim em $\vec{r}(t_1)=P$. Então, supondo que $\vec{F}$ é convervativo, isto é, $\vec{F}=\vec{\nabla}\varphi$, temos
\begin{eqnarray*}
...
\end{eqnarray*}
Reciprocamente, dado que a expressão \eqref{TeoFundCalculo} é válida para qualquer curva $C$ que liga dois pontos na região, definimos a curva $C$ que liga $(x,y,z)$ a $(x+\Delta x,y,z)$ pelo segmento de reta parametrizado por $\vec{r}=t\vec{i}+y\vec{j}+z\vec{k}$, $x\leq t\leq x+\Delta x$ e a função
$$
\varphi(x,y,z)=\int_C \vec{F}\cdot d\vec{r}.
$$
Mostraremos que $\phi$ é o potencial do campo $\vec{F}=F_1\vec{i}+F_2\vec{j}+F_3\vec{k}$, ou seja, $\vec{F}=\vec{\nabla}\varphi$. De fato,
\begin{eqnarray*}
\varphi(x+\Delta x,y,z)-\varphi(x,y,z)&=&\int_C \vec{F}\cdot d\vec{r}\\
&=&\int_x^{x+\Delta x} \vec{F}\cdot \vec{r}' dt \\
