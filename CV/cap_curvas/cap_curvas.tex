...
\begin{center}
    \includegraphics[scale=.75]{./cap_curvas/figs/vetores_ijk}
...
O domínio natural de uma função vetorial é formado pelo conjunto de parâmetro possíveis tal que a função ainda seja real.
\begin{ex}
 O domínio da função vetorial $\vec{r}(t)=\frac{1}{\sqrt{t-1}}\vec{i}+\frac{1}{2-t}\vec{j}$ é formado pelo parâmetros $t$ tal que $t-1>0$ e $2-t\neq 0$, pois não há divisão por zero nem raíz de número negativo no conjunto dos números reais. Logo, $D=\left\{t\in\mathbb{R}; t>1 \text{ e } t\neq 2\right\}$
\end{ex}
...
\vec{r}~\!'(t)=\frac{d\vec{r}(t)}{dt}&=&\frac{d x(t)}{dt}\vec{i}+\frac{d y(t)}{dt}\vec{j}+\frac{d z(t)}{dt}\vec{k}\label{defder}\\
...
\end{enumerate}
\end{teo}
\begin{proof} Os dois primeiros itens podem ser obtidos diretamente de (\ref{defder}). A verificação fica a cargo do leitor. O item três pode ser obtido de  uma aplicação da regra da cadeia a (\ref{defder})\index{derivada do produto de um escalar por um vetor}:
...
\begin{center}
    \includegraphics[scale=.5]{./cap_curvas/figs/helice}
...
\begin{itemize}
 ...
 \item[b)] $\vec{r}=\cos(\pi t)\vec{i}-\ln(t)\vec{j}+\sqrt{t-2} \vec{k}$; $t_0=3$.
\end{itemize}
\end{exer}
...
\begin{figure}%{0.35\textwidth}
\begin{center}
    \includegraphics[scale=.75]{./cap_curvas/figs/vetor_tangente_circunferencia}
...
\begin{figure}%{0.35\textwidth}
\begin{center}
    \includegraphics[scale=.75]{./cap_curvas/figs/helice_TNB}
...
\subsection*{Exercícios resolvidos} % Triedro de Frenet-Serret
...
\end{exeresol}
\begin{resol}
  Primeiro calculamos $\vec{v}(t)=\frac{d\vec{r}}{dt}$:
...
portanto $$\frac{d\vec{T}}{dt} = \frac{36}{(12+18t^2)^{3/2}}\left[  t \vec{i} + (1-t) \vec{j} - (1+t)\vec{k} \right] $$
    o que implica $$\left\|\frac{d\vec{T}}{dt}\right\|= \frac{36}{(12+18t^2)^{3/2}} \left\| t \vec{i} + (1-t) \vec{j} - (1+t)\vec{k} \right\| = \frac{36}{(12+18t^2)^{3/2}} \sqrt{ 3t^2 + 2} $$
    Então
     $$\vec{N} = \frac{\frac{d\vec{T}}{dt}}{\|\frac{d\vec{T}}{dt}\|} = \frac{t}{\sqrt{3t^2+2}} \vec{i} + \frac{1-t}{\sqrt{3t^2+2}} \vec{j} - \frac{1+t}{\sqrt{3t^2+2}} \vec{k}.$$
     Finalmente $ \vec{B} = \vec{T} \times \vec{N}$ é calculado via \begin{eqnarray*}
      \vec{B} = \vec{T} \times \vec{N} &=&
...
já que $\sqrt{12+18t^2} \sqrt{3t^2+2}= \sqrt{6}(2+3t^2)$
\end{resol}
...
\subsection*{Exercícios} %Triedro de Frenet-Serret
...
\begin{exer}\label{exer_formula_binormal}
 ...
 $$
 \vec{B}(t)=\frac{\frac{d\vec{r}}{dt}(t)\times \frac{d^2\vec{r}}{dt^2}(t)}{\|\frac{d\vec{r}}{dt}(t)\times \frac{d^2\vec{r}}{dt^2}(t)\|}.
 $$
\end{exer}
\begin{exer}
...
\begin{figure}
\begin{center}
    \includegraphics[scale=.75]{./cap_curvas/figs/circulo_curvatura_parabola}
...
\begin{exeresol} Calcule a curvatura, raio de curvatura e o módulo da torção para a hélice circular $\vec{r}(t)=\cos(t)\vec{i}+\sin(t)+t\vec{k}$.
\end{exeresol}
Primeiro calculamos a derivada de $\vec{r}(t)$:
\begin{resol}
$$
\frac{d\vec{r}(t)}{dt}=-\sin(t)\vec{i}+\cos(t)\vec{j}+\vec{k},
$$
...
$$
\vec{T}(t)=-\frac{\sin(t)}{\sqrt{2}}\vec{i}+\frac{\cos(t)}{\sqrt{2}}\vec{j}+\frac{1}{\sqrt{2}}\vec{k},
$$
Para calcular o módulo da torção, calculamos a derivada do vetor tangente unitário:
$$
\frac{d\vec{T}(t)}{dt}=-\frac{\cos(t)}{\sqrt{2}}\vec{i}-\frac{\sin(t)}{\sqrt{2}}\vec{j},
$$
cujo módulo é dado por:
$$
...
\end{resp}
 ...
 \begin{figure}
\begin{center}
    \includegraphics[scale=.75]{./cap_curvas/figs/curvatura_torcao}
 \caption{Curvatura e Torção}\label{Curvatura_torcao_1}
  \end{center}
\end{figure}


\section{Fórmulas alternativas para curvatura e torção}
...
Desta forma, temos:
$$\kappa(t)=\frac{\|\vec{r}~\!'(t)\times \vec{r}~\!''(t)\|}{\|\vec{r}~\!'(t)\|^3}=\frac{ab}{\left[a^2\sin^2(t)+b^2\cos^2(t)\right]^{3/2}}$$
...
\begin{ex} \label{exemplo_sincosf} Consideremos agora, a curva gerada pelas seguintes equações paramétricas:
...
$$\kappa=\frac{\left\|\vec{r}~\!'(t)\times \vec{r}~\!''(t)\right\|}{\|\vec{r}~\!'(t)\|^3}.$$
...
\begin{eqnarray*}\kappa &=& \frac{\left\|\vec{r}~\!'(t)\times \vec{r}~\!''(t)\right\|}{\|\vec{r}~\!'(t)\|^3}\\
...
&=& \frac{\left\|\frac{d\vec{B}}{dt} \right\|}{\left\|\vec{r}~\!'(t)\right\|}\\
\end{eqnarray*}
...
e
\begin{eqnarray*}
\|\vec{r}~\!'(t)\|&=&\left\|-aw\sin(wt)\vec{i}+aw\cos(wt)\vec{j}+c\vec{k}\right\|\\
&=&\sqrt{c^2+a^2w^2}
\end{eqnarray*}
...
$$\kappa=\frac{\left\|\vec{r}~\!'(t)\times \vec{r}~\!''(t)\right\|}{\|\vec{r}~\!'(t)\|^3}=\frac{\left\|\left(\vec{i}+f'(t)\vec{j}\right)\times f''(t)\vec{j}~\!\right\|}{\|~\!\vec{i}+f'(t)\vec{j}\|^3}$$
...
\begin{resol}
  ...
  {\it Obs:}
 A parametrização $\vec{r}=a\sin (t) + b\cos (t)$ descreve a elipse, mas não satisfaz a condição de velocidade escalar constante, pelo que pode ser usada para obter a curvatura, mas não representa a cinética do problema. Por isso, usamos a curvatura da elipse ao realizar os cálculos e conectar aceleração normal com velocidade escalar.
\end{resol}
\begin{exeresol}
...
\end{exer}
\begin{exer} Através da fórmula da curvatura dada por $\kappa(t)=\frac{\|\vec{v}(t)\times \vec{a}(t)\|}{v(t)^3} $, conclua que se a aceleração normal de uma partícula em movimento é zero para todo $t$, então a partícula se move ao longo de uma reta.
\end{exer}
