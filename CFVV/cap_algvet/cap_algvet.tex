...
\chapter{Álgebra vetorial}\label{chap:algvet}\index{álgebra vetorial}
...
\begin{obs}Observamos que a propriedade associativa dada por (\ref{defelass}) permite que se escreva a soma de três vetores $\vec{u}+\vec{v}+\vec{w}$ sem risco de ambiguidade. A propriedade (\ref{defeldist3}) é algumas vezes chamada de associatividade, no entanto, é cauteloso observar que ela não estabelece a associatividade de uma operação, já que o produto de escalares é uma operação distinta do produto de um escalar por um vetor. A propriedade (\ref{defelnulo}) garante a existência de um vetor nulo que funciona com um elemento neutro da soma vetorial. \end{obs}
...
\begin{equation}\label{defnulo}\vec{0}=0 ~\! \vec{i}+0~\!  \vec{j}+0~\!  \vec{k}= ~\! \langle 0,0,0 \rangle.\end{equation}
...
\begin{equation}\label{defnorma}
\|\vec{v}\|=\sqrt{v_1^2+v_2^2+v_3^2}.
\end{equation}

\subsection*{Exercícios resolvidos}

\begin{exeresol} \label{exenorm1}
 Aplique a definição para calcular a norma do vetor $\vec{x} = 12 \vec{i} + 16 \vec{j} + 8 \vec{k}$. 
\end{exeresol}
\begin{resol}
$$\|\vec{x}\| = \sqrt{(12)^2+(16)^2+8^2} = \sqrt{(3\cdot4)^2 + (4\cdot4)^2 + (2\cdot 4)^2} = \sqrt{4^2(3^2 + 4^2 + 2^2)} = 4\sqrt{29}$$
\end{resol}

\begin{exeresol} \label{exenorm2}
 Aplique a definição para calcular, para $t>0$, a norma do vetor
 $\vec{x} = t^3 \vec{i} + t^4 \vec{j} + 2t^4 \vec{k}$.
\end{exeresol}

\begin{resol}
$$\|\vec{x}\| = \sqrt{t^6 + t^8 + 4t^8} = \sqrt{t^6(1 + 5t^2)}=t^3\sqrt{1 + 5t^2}$$
\end{resol}
...
\begin{exer}
O uso da propriedade de homogeneidade (\ref{propnormahom}) permite simplificar muito o cálculo de versores $\hat{x}$. Para tal, seja $\vec{x} = 12 \vec{i} + 16 \vec{j} + 8 \vec{k}$. Mostre que $\vec{x} = 4 ( 3 \vec{i} + 4 \vec{j} + 2 \vec{k} ) $ e aplique tal propriedade para concluir que $\hat{x}$ é o versor de $3 \vec{i} + 4 \vec{j} + 2 \vec{k}$.
\end{exer}
Resp:
$$\hat{x} = \frac{ 3 \vec{i} + 4 \vec{j} + 2 \vec{k} }{\sqrt{3^2 + 4^2 + 2^2}}$$

\begin{exer}
Mais uma vez use a propriedade de homogeneidade (\ref{propnormahom}) para simplificar  o cálculo de versores $\hat{x}$. Para $t > 0$, seja $\vec{x} = t^3 \vec{i} + t^4 \vec{j} + 2t^4 \vec{k}$. Mostre que $\vec{x} = t^3 (  \vec{i} + t \vec{j} + 2t \vec{k} ) $ e aplique tal propriedade para concluir que $\hat{x}$ é o versor de $\vec{i} + t \vec{j} + 2t \vec{k}$.
\end{exer}
Resp:
$$\hat{x} = \frac{ \vec{i} + t \vec{j} + 2t \vec{k} }{\sqrt{1 + 5t^2}}$$
...
\item[c)] Seja $d$ a distância entre dois pontos sobre a superfície terrestre. Use a lei dos cossenos (expressa na figura \ref{leicossenos}) para mostrar que distância $\delta$ sobre a superfície esférica entre esses mesmos dois pontos é dada por
...
\begin{exer}\label{exort2} Sejam $\vec{u}=2\vec{i}+\vec{j}+\vec{k}$ e $\vec{v}=\vec{i}-\vec{j}-\vec{k}$. Mostre que estes vetores são ortogonais entre si. Encontre dois vetores unitários distintos ortogonais tanto a  $\vec{u}$ como a $\vec{v}$.
\end{exer}
